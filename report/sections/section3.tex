...
\midrule
\endhead
Geometry awareness & 3DGS thiếu surface/collision rõ ràng dù render tốt. & Phải sinh geometry proxy từ \code{.ply/.splat}, không chỉ giữ dữ liệu render.\\
Occlusion readiness & AR cần vật thể thật che đúng vật thể ảo. & Phải xuất occlusion mesh ở định dạng phổ biến như GLB.\\
Navigation readiness & Agent hoặc logic di chuyển cần vùng walkable. & Phải có khả năng bake navmesh với tham số agent và cảnh báo khi rỗng.\\
Scale/coordinate consistency & Scene tái tạo có thể sai scale, sai up axis hoặc lệch transform. & Phải có recipe căn chỉnh scale/up axis và bake transform vào dữ liệu.\\
WebAR deliverability & Runtime web cần artifact nhẹ, dễ mở và tự chứa. & Phải đóng gói \artifact{scene.sog}, mesh, viewer và metadata thành bundle.\\
Performance and lightweight output & Cảnh 3DGS có thể có rất nhiều splat, mesh quá nặng sẽ khó dùng. & Phải có GPU path, metric thời gian, triangle count và tỷ lệ dung lượng.\\
